...
object ImoocLogCleanAndReduceYARN {

  def main(args: Array[String]): Unit = {

    val sc = new SparkConf().setAppName("kafka-stream")
    ...
    /*
    get kafka stream by direct stream
     */
    ...
    /*
       clean the stream data into clicklog bean
     */
    val cleanStream = kafkaStream.map(
      line => {
        val arr = line.value().split("\t")
        println("date:-----------------"+arr(1)+" arr:"+arr.mkString(","))
...
/*
   save each rdd into the hbase ,increase the column "click_count"
 */
  ...
  //baidu?q=dd
    /*
       save each rdd into the hbase ,increase the column "referer_count"
